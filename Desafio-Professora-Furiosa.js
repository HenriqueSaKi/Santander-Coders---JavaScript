...
function acontece(alunosSemana,qtdeMin){
  var somaAluno = 0;
  for(var i = 0; i < alunosSemana.length; i++){
    if(alunosSemana[i]<=0){
      somaAluno = somaAluno + 1;
    }
  }
  if(somaAluno>=qtdeMin){
    return true;
  }
  else{
    return false;
...
function aberturas(alunosDoDiaDaSemana,qtdeMin){
var result = [];
 for(var x = 0; x < alunosDoDiaDaSemana.length; x++){
    result.push(acontece(alunosDoDiaDaSemana[x],qtdeMin));
  }
  return result;
}
